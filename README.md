...
│   │   ├── app/
│   │   │   ├── (app)/                                # หน้าหลังล็อกอิน
│   │   │   │   ├── receiving/                        # รับสินค้า (หน้าหลัก operator)
...
│   │   │   │   ├── units.ts, catalog-history.ts
│   │   │   │   └── …
│   │   │   ├── catalog/, db/, navigation/, users/
│   │   │   └── api/, supabase/, types.ts, utils/
│   │   └── middleware.ts
│   ├── scripts/
│   │   ├── seed-pins.ts, import-from-csv.ts, import-transactions.ts
│   │   ├── build-units-from-transactions.ts
│   │   ├── apply-migrations.ts                       # DEPLOY_ENV=production + DATABASE_URL
│   │   ├── deploy-phase1-setup.ts, verify-supabase.ts, print-vercel-env.ts
│   │   ├── load-deploy-env.ts, reset-user-pins.ts, restore-team-users.ts
...
| `intake_slips` | หัวใบรับสินค้า (วันที่, ร้าน, หมายเหตุ, audit สร้าง/แก้) |
| `transactions` | รายการรับของ (อ้างอิง `slip_id` เมื่อมีใบ) |
...
npm run import:post   # ต้องมี DATABASE_URL
```

**ฐานมีข้อมูลแล้ว (staging / production append):** ดู [`docs/historical-import.md`](docs/historical-import.md)

```bash
DEPLOY_ENV=staging npm run import:transactions -- --dry-run --append --file="/path/to/export.xlsx"
DEPLOY_ENV=staging npm run import:transactions -- --append --file="/path/to/export.xlsx"
DEPLOY_ENV=staging npm run import:post
```

ต้นทาง default: `Backup/DB File/RM Amazing Nongkhai - DB.xlsx` — สำรองก่อนนำเข้า: `./scripts/backup-before-import.sh`
...
npm run import:csv
npm run import:transactions
npm run import:post      # backfill intake_slips + setval (ต้องมี DATABASE_URL)
npm run export:db        # snapshot จำนวนแถว (DEPLOY_ENV=production|staging)
npm run build:units      # สร้างหน่วยจากประวัติ transaction
npm run db:migrate       # รัน migration ผ่าน DATABASE_URL (production|staging)
npm run deploy:verify    # ตรวจ Supabase (DEPLOY_ENV=production|staging|local)
npm run deploy:setup     # seed + import ครั้งแรกบน production
npm run reset:user-pins  # รีเซ็ต PIN ตาม script
```
...
- [`web/src/lib/services/data.ts`](web/src/lib/services/data.ts) — Supabase data layer (รวม rename รหัสสินค้า)
- [`web/src/lib/services/intake-slips.ts`](web/src/lib/services/intake-slips.ts) — บันทึก/ลบใบรับสินค้า
...
| GET/POST | `/api/transactions` | ดึง/บันทึกรายการ (POST รองรับ `slipId`, `slipNote`) |
| POST | `/api/transactions/replace` | ทับรายการ legacy (ไม่มี slip) |
| DELETE | `/api/transactions/batch` | ลบทั้งวัน+ร้าน หรือทั้งใบ (`slipId`) |
| GET | `/api/transactions/slips` | รายการใบในวัน+ร้าน (+ `canEdit`) |
| GET/PATCH/DELETE | `/api/transactions/slips/[id]` | เมตาใบ / ลบใบ |
| GET | `/api/transactions/slip-meta` | สรุปใบ (legacy) |
| PATCH | `/api/items/[code]` | แก้สินค้า (admin เปลี่ยนรหัสได้) |
